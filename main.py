...
from utils import (
    ...
    get_queues,save_message, 
    send_message, update_env_file,
    ...
    while True:
        try:
            ...
            try:
                ...
                await watch_for_connection(watchdog_queue, ConnectionStatus.ALIVE, ConnectionSource.TIMEOUT)
    ...
    while True:
        try:
            ...
            try:
                while True:
                    if not is_auth:
                        await watch_for_connection(watchdog_queue, ConnectionStatus.ALIVE, ConnectionSource.BEFORE_AUTH)
...
async def ping_server(reader: asyncio.StreamReader, gui_class: Any, status_updates_queue: asyncio.Queue) -> bool:
    start_ping_time = 0
...
async def authorize(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, token: str) -> str:
    try:
        if token:
